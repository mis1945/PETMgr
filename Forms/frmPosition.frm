...
Option Explicit
Private Const pxeModuleName = "frmPosition"
...
Private Sub oDriver_InitValue()
   ...
   Dim ldPositionID As String
   
   lsOldProc = "oDriver_InitValue"
   On Error GoTo errProc
   
   'she 2022-10-22
   'hardcode for position id dahil sa encoded Posisiontid from honda cars during the merging
   ' GetNextCode("Position", "sPositnID", False, oApp.Connection)
   ...
   lsSQL = "SELECT" & _
            ...
            " FROM Position" & _
            " WHERE cRecdStat = " & strParm(xeRecStateActive) & _
            " AND sPositnID NOT LIKE 'A%'" & _
            " ORDER BY sPositnID DESC Limit 1"
   ...
   If loData.EOF Then
      ldPositionID = "0001"
   Else
      ldPositionID = loData("sPositnID") + 1
   End If
   If Not oDriver.SetValue(0, ldPositionID) Then Exit Sub
